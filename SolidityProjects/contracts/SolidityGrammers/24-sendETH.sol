...
/*
Solidity有三种方法向其他合约发送ETH，分别是：transfer()，send()和call()，其中call()是被鼓励的用法
...
contract SendETH{
    ...
    constructor() payable{} // 构造函数，payable使得部署的时候可以转eth进去
    receive() external payable{} // receive方法，接收eth时被触发
   ...
   // 用transfer()发送ETH
   function transferETH(address payable _to,uint256 amount) external payable{ // 注意里面的_to填ReceiveETH合约的地址，amount是ETH转账金额：
    _to.transfer(amount);
   }
